...
#include <Adafruit_BMP085.h> //Se usa este modelo del sensor
...
void setup() {
  ...
  Serial.begin(115200);
  bmp.begin(0x77); //Inicializar comunicación I2C con el sensor barométrico 
  ...
  //*****************************
  // Cert leer archivo
  File file4 = SPIFFS.open("/4d90-certificate.pem.crt", FILE_READ);
  if (!file4) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
  ...
  File file6 = SPIFFS.open("/4d90-private.pem.key", FILE_READ);
  if (!file6) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
